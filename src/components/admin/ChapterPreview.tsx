...
export function ChapterPreview({
  storyImgs, chapters, gtLg
}: { storyImgs: string[], chapters: Chapter[], gtLg: boolean}) {
  ...
  return <div className="bg-white flex gap-x-8 relative py-32 xl:mx-auto xl:px-16 px-8 md:px-16">
    {gtLg &&
      <div className="w-[60%] mr-8">
        {chapters.map((chapter, chapterIndex) => <div className="w-fit mx-auto grid items-center h-[100vh]"
          key={`sticky-img-${chapterIndex}`}
        >
          {storyImgs[chapterIndex] ? <img
            src={storyImgs[chapterIndex]}
            className="max-w-full max-h-[75vh] object-contain" /> : 'KEIN BILD'}
        </div>
        )}
      </div>}
        ...
        >
          {!gtLg &&
            ...
            >
              {storyImgs[chapterIndex] ? <img
                src={storyImgs[chapterIndex]}
                className="max-h-[50vh] lg:max-h-[75vh] max-w-[100%] object-contain" /> : 'KEIN BILD'}
            </div>}
          <div
            className="lg:max-w-[min(65ch,40%)]"
            dangerouslySetInnerHTML={{
              __html: '<div class="flex flex-col gap-4 items-center lg:items-start text-center lg:text-left">' + chapter.children.map((child) => {
